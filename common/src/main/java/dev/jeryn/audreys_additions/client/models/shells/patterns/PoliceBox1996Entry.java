...
public class PoliceBox1996Entry extends ShellEntry {

    public PoliceBox1996Entry(ShellModel shellModel, ShellDoorModel shellDoorModel) {
    ...
    public ShellModel getShellModel(ShellPattern shellPattern) {
        if (isSpecialShellPattern(shellPattern)) {
            return AUDModelRegistry.policeBox03;
        }
    ...
    public ShellDoorModel getShellDoorModel(ShellPattern shellPattern) {
        if (isSpecialShellPattern(shellPattern)) {
            return AUDModelRegistry.policeBox03Door;
        }
